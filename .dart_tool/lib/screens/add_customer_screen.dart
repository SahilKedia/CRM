...
class _AddCustomerScreenState extends State<AddCustomerScreen> {
  ...
  // Additional Info (customer-level, optional)
  ...
  // Pick image for Additional Info section (customer-level, optional)
            ...
            if (_additionalInfoImageFile != null || _existingAdditionalInfoImageUrl != null)
              ListTile(
                ...
                onTap: () {
                  Navigator.pop(context);
                  setState(() {
                    _additionalInfoImageFile = null;
                    _existingAdditionalInfoImageUrl = null;
                  });
                },
              ),
            const SizedBox(height: 8),
  ...
  // Tapping any thumbnail (network image or locally picked file) opens a
  // full screen, zoomable viewer so the photo can be seen clearly.
...
// ---------- NOTIFICATION ----------
  Future<void> _handleReminderNotification({
    ...
    try {
      ...
      // If a reminder was previously scheduled at a different date/time
      // than the new one (or the reminder is being cleared entirely),
      // cancel it first - otherwise the old OS notification (a different
      // ID, since IDs are keyed by customerId + date) keeps firing
      // alongside, or instead of, the new one.
      if (previousReminderDate != null &&
          (reminderDateTime == null ||
              !previousReminderDate.isAtSameMomentAs(reminderDateTime))) {
        await notificationService.cancelReminder(customerId, previousReminderDate);
      }
  ...
  Future<void> _addCustomer() async {
    ...
    try {
      ...
      // Needed so _handleReminderNotification can cancel the *old* scheduled
      // notification if the reminder's date/time changed or was cleared.
      DateTime? previousReminderDate;
      if (_isEditMode) {
        final existingReminder = widget.customerToEdit?['reminder'];
        if (existingReminder is Map) {
          final existingStatus = existingReminder['status']?.toString() ?? 'pending';
          final existingDateStr = existingReminder['date']?.toString();
          if (existingStatus == 'pending' &&
              existingDateStr != null &&
              existingDateStr.isNotEmpty) {
            previousReminderDate = DateTime.tryParse(existingDateStr)?.toLocal();
          }
        }
      }  
      ...
      // additional info text (used in both edit and new-customer branches below)
      ...
      if (_isAddingVisit && _isEditMode) {
        ...
        // (backend trims it, so '' becomes undefined there — matches the pattern
        // used for birthday/anniversary/profession/community).
        ...
        // Only send if user actually typed something (fully optional field)
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            ...
            _handleReminderNotification(
              customerId: customerId,
              customerName: customerName,
              previousReminderDate: previousReminderDate,   // ← add this line
              reminderDateTime: reminderDateTime,
              reminderMessage: reminderMessageStr,
            );
          }
                                ...
                                child: _buildDropdownField<String>(
                                  ...
                                  items: _titleOptions
                                      .map((t) => DropdownMenuItem(
                                            value: t,
                                            child: Text(t, overflow: TextOverflow.visible, softWrap: false),
                                          ))
                                      .toList(),
  ...
  // Professional card wrapper used to group each form section with a
  // consistent white card, soft shadow, rounded corners and an icon + title
  // header (replaces the old plain-text-only section headers).
  ...
  Widget _buildCustomerImagePicker() {
    ...
    return GestureDetector(
      // Tap the photo itself to view it full-screen (when one exists);
      // tap the small camera badge to change/remove it.
      onTap: imageProvider != null
          ? () {
              if (_customerImageFile != null) {
                _openImageViewer(files: [_customerImageFile!], initialIndex: 0, title: 'Customer Photo');
              } else if (_existingCustomerImageUrl != null) {
                _openImageViewer(
                  networkUrls: [ApiService.getImageUrl(_existingCustomerImageUrl!)],
                  initialIndex: 0,
  ...
  // Additional Info section — optional text note + optional single image
      ...
      children: [
        ...
        InkWell(
          onTap: imageProvider == null
              ...
              : () {
                  if (viewerFile != null) {
                    _openImageViewer(files: [viewerFile], initialIndex: 0, title: 'Additional Info Photo');
                  } else if (viewerUrl != null) {
                    _openImageViewer(networkUrls: [viewerUrl], initialIndex: 0, title: 'Additional Info Photo');
                  }
                },
          borderRadius: BorderRadius.circular(10),
          child: Container(
            ...
            child: imageProvider == null
                ? Column(
                    children: [
                      ...
                      const Text(
                        'Add Additional Info Image (optional)',
                        style: TextStyle(color: AppColors.textSecondary, fontSize: 12),
                      ),
                    ],
                  )
                : Column(
                    children: [
                      ...
                      GestureDetector(
                        ...
                        child: const Text(
                          'Tap image to view · tap here to change or remove',
                          style: TextStyle(color: AppColors.textSecondary, fontSize: 12),
                        ),
                      ),
                    ],
        ...
        DropdownButtonHideUnderline(
          child: DropdownButtonFormField<T>(
            ...
            decoration: InputDecoration(
              ...
              // In dense (narrow) fields, such as the Title picker, the prefix
              // icon plus text plus dropdown arrow used to fight for space and
              // clip the label (e.g. "Mr." wouldn't fully render). Dropping
              // the icon and tightening the padding fixes that.
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, i) {
                ...
                return Padding(
                  ...
                  child: GestureDetector(
                    // Tap a thumbnail to open it full-screen, clearly
                    // visible and zoomable, swiping through the rest.
                    onTap: () => _openImageViewer(
                      networkUrls: imageUrls.map((u) => ApiService.getImageUrl(u)).toList(),
                      initialIndex: i,
                      title: label,
                    ),
                    child: ClipRRect(
          ...
          Container(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                final url = ApiService.getImageUrl(imageUrls[index]);
                return GestureDetector(
                  // View previously uploaded images full-screen on tap.
                  onTap: () => _openImageViewer(
                    networkUrls: imageUrls.map((u) => ApiService.getImageUrl(u)).toList(),
                    initialIndex: index,
                    title: label,
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return Stack(
                  children: [
                    GestureDetector(
                      // View newly picked local images full-screen on tap.
                      onTap: () => _openImageViewer(files: images, initialIndex: index, title: label),
                      child: Container(
                        width: 100,
                        height: 100,
                        margin: const EdgeInsets.only(right: 8),
...
// Used whenever a customer/visit photo (network or freshly-picked local file)
// is tapped, so the photo can be seen clearly, with support for swiping
// between multiple photos and double-tap / pinch zoom.
// ============================================================================
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _itemCount > 1
              ? '${widget.title ?? 'Photo'} (${_currentIndex + 1}/$_itemCount)'
              : (widget.title ?? 'Photo'),
      ...
      body: GestureDetector(
        ...
        child: PageView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return GestureDetector(
              // Absorb taps on the image itself so tapping the photo doesn't
              // immediately close the viewer, while tapping the surrounding
              // black area (handled by the parent GestureDetector) still does.
              onTap: () {},
