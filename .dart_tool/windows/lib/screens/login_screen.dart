...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _loadSavedCredentials() async {
    try {
      ...
      if (mounted && credentials['email'] != null && credentials['email']!.isNotEmpty) {
        setState(() {
          _rememberMe = true;
  ...
  Future<void> _handleLogin() async {
    ...
    try {
      ...
      if (response["success"] == true) {
        ...
        // Save login data (token and user data)
        // Assuming your API returns a token, adjust as needed
        final token = response["token"] ?? "dummy_token_${DateTime.now().millisecondsSinceEpoch}";
  ...
  Future<void> _handleVerifyOtp() async {
    ...
    try {
      ...
      if (response["success"] == true) {
        // Save login data for employee (no remember me)
        final token = response["token"] ?? "dummy_token_${DateTime.now().millisecondsSinceEpoch}";
        final userData = response["user"] as Map<String, dynamic>;
  ...
  void _navigateAfterLogin(Map<String, dynamic> userData) {
    ...
    Widget destination;
    if (role == "employee") {
      destination = EmployeeDashboardScreen(user: userData);
    } else if (role == "admin") {
      destination = DashboardScreen(user: userData);
    } else if (role == "superadmin") {
      destination = SuperAdminDashboardScreen(user: userData);
    } else {
  ...
  // ---------------------------------------------------------------
  // EMPLOYEE FORM (email -> OTP -> verify)
  // ---------------------------------------------------------------
