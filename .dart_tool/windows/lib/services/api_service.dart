...
Future<Map<String, dynamic>> markReminderCompleted(String customerId) async {
  try {
    ...
    String message = 'Failed to update reminder status';
...
Future<Map<String, dynamic>> createCustomerWithImages(
  ...
  File? customerImage,
  File? additionalInfoImage, // ✅ ADD THIS NEW PARAMETER
}) async {
  try {
    final token = await _getToken();
    if (token == null) {
    ...
    final Map<String, String> fieldMapping = {
      ...
      'additionalInfo': customerData['additionalInfo']?.toString() ?? '', // ✅ ADD THIS
    };
    ...
    // Add customer image if provided
    ...
    // ✅ NEW: Add additional info image if provided
    if (additionalInfoImage != null) {
      formData.files.add(
        MapEntry(
          'additionalInfoImage',
          await MultipartFile.fromFile(
            additionalInfoImage.path,
            filename: 'additional_info_${DateTime.now().millisecondsSinceEpoch}.jpg',
          ),
        ),
      );
      print('📸 Added additional info image');
    }
...
//     // Check if we need to use multipart form data (if any files are provided)
//     final bool hasCustomerImage = customerImage != null;
//     final bool hasAdditionalInfoImage = additionalInfoImage != null;

//     dynamic requestData = customerData;
//     String contentType = 'application/json';

//     // If we have ANY files, use multipart form data
//     if (hasCustomerImage || hasAdditionalInfoImage) {
//       // Create FormData from customerData fields
...
//       // ✅ NEW: Add additional info image if provided
//       if (hasAdditionalInfoImage) {
//         formData.files.add(
//           MapEntry(
//             'additionalInfoImage', // This must match your backend field name
//             await MultipartFile.fromFile(
//               additionalInfoImage!.path,
//               filename: 'additional_info_${DateTime.now().millisecondsSinceEpoch}.jpg',
//             ),
...
// }
   ...
   // In api_service.dart - Update the updateCustomer method

Future<Map<String, dynamic>> updateCustomer(
  String id,
  Map<String, dynamic> customerData, {
  File? customerImage,
  File? additionalInfoImage, // ✅ Already added as parameter
}) async {
  try {
    final token = await _getToken();
    if (token == null) {
    ...
    if (hasCustomerImage || hasAdditionalInfoImage) {
      // Create FormData from customerData fields
      // Use FormData.fromMap to handle fields properly
      final formData = FormData();

      // Add all fields from customerData
      ...
      // ✅ FIX: Add additional info image if provided
      if (hasAdditionalInfoImage) {
        formData.files.add(
          MapEntry(
            'additionalInfoImage', // This must match your backend field name
